...
func openStubDB(t *testing.T, state *stubState) *sql.DB {
	...
	registerStubDriverOnce.Do(func() {
		sql.Register("feedback-repository-stub", &stubDriver{})
	})
	...
	db, err := sql.Open("feedback-repository-stub", dsn)
	if err != nil {
...
func TestUpdateBatchRollsBackOnNotFound(t *testing.T) {
	...
	err := repo.UpdateBatch(context.Background(), []feedbackdomain.Feedback{
		{FeedbackType: "like", UserID: "u-1", ItemID: "i-1"},
		{FeedbackType: "like", UserID: "u-1", ItemID: "i-2"},
	})
	if err != feedbackdomain.ErrFeedbackNotFound {
...
func TestDeleteBatchAllowsDuplicateKeys(t *testing.T) {
	state := &stubState{
		execResults: []stubExecResult{
			{rowsAffected: 1},
		},
	...
	err := repo.DeleteBatch(context.Background(), []feedbackdomain.Key{
		{FeedbackType: "like", UserID: "u-1", ItemID: "i-1"},
		{FeedbackType: "like", UserID: "u-1", ItemID: "i-1"},
	})
	if err != nil {
	...
	if len(state.execArgCounts) != 1 || state.execArgCounts[0] != 3 {
		t.Fatalf("expected delete to execute with 1 unique composite key, got %v", state.execArgCounts)
	}
}
